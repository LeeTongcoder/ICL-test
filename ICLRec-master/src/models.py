...
class KMeans(object):
    def __init__(self, num_cluster, seed, hidden_size, gpu_id=0, device="cpu"):
        ...
        """
        self.seed = seed
        self.num_cluster = num_cluster  # 256
    ...
    def train(self, x):
        ...
        # get cluster centroids获得聚类中心
        centroids = faiss.vector_to_array(self.clus.centroids).reshape(self.num_cluster, self.hidden_size)  # (256, 64)选取256个聚类中心
...
class SASRecModel(nn.Module):
    ...
    def add_position_embedding(self, sequence):
        ...
        position_ids = position_ids.unsqueeze(0).expand_as(sequence)    # (256, 50)，每一行都是0-49顺序排列，作为项目的位置标识
        item_embeddings = self.item_embeddings(sequence)    # (256, 50, 64)nn.Embedding输出64维,注意用的是原始项目id
        position_embeddings = self.position_embeddings(position_ids)    # (256, 50, 64)nn.Embedding输出64维，
    ...
    def forward(self, input_ids):   # input_id是经过补0后维度为256*50的项目序列（去掉3个尾巴）

        attention_mask = (input_ids > 0).long() # 将有数值的全部变成1
        extended_attention_mask = attention_mask.unsqueeze(1).unsqueeze(2)  # torch.int64，扩展维度为(256, 1, 1, 50)
        max_len = attention_mask.size(-1)   # 这个完全可以args.max_seq_length
        attn_shape = (1, max_len, max_len)
        subsequent_mask = torch.triu(torch.ones(attn_shape), diagonal=1)  # torch.uint8, torch.triu返回上三角矩阵，
        subsequent_mask = (subsequent_mask == 0).unsqueeze(1)   # 值为0为True，其他权威False，扩展维度为[1, 1, 50, 50]
        subsequent_mask = subsequent_mask.long()    # .long()后True用1表示，False用0表示
        # # 直接变成下三角感觉也可以
        # subsequent_mask = torch.tril(torch.ones(1, 1, max_len, max_len), diagonal=0).long()
        if self.args.cuda_condition:
            subsequent_mask = subsequent_mask.cuda()

        extended_attention_mask = extended_attention_mask * subsequent_mask     # 点乘维度为(256, 1, 50, 50)
        extended_attention_mask = extended_attention_mask.to(dtype=next(self.parameters()).dtype)  # fp16 compatibility
        extended_attention_mask = (1.0 - extended_attention_mask) * -10000.0

        sequence_emb = self.add_position_embedding(input_ids)
        # 经过自注意力和intermediate模块，返回有两个(256, 50, 64)tensor的列表
