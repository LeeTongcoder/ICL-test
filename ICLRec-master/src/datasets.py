...
class RecWithContrastiveLearningDataset(Dataset):
    ...
    def _one_pair_data_augmentation(self, input_ids):
        ...
        augmented_seqs = []
        for i in range(2):
            augmented_input_ids = self.base_transform(input_ids)    # 调用data_augumentation，选择两种不同的方式，返回
            pad_len = self.max_len - len(augmented_input_ids)
            augmented_input_ids = [0] * pad_len + augmented_input_ids

            augmented_input_ids = augmented_input_ids[-self.max_len :]  # 同样要补0

            assert len(augmented_input_ids) == self.max_len

            cur_tensors = torch.tensor(augmented_input_ids, dtype=torch.long)   # 转为tensor
    ...
    def _data_sample_rec_task(self, user_id, items, input_ids, target_pos, answer):
        ...
        target_neg = []
        seq_set = set(items)
        for _ in copied_input_ids:  # 每一个正样本都要要获得一个对应的负样本，保存在target_neg列表中中
            target_neg.append(neg_sample(seq_set, self.args.item_size))

        pad_len = self.max_len - len(copied_input_ids)
        copied_input_ids = [0] * pad_len + copied_input_ids     # 设置每个用户交互都是max_seq_length=50个，其余的在前面用0补全
    ...
    def __getitem__(self, index):
        ...
        if self.data_type == "train":
            input_ids = items[:-3]  # 这个input为什么要去掉后面3个呢？？？
            target_pos = items[1:-2]    # 设置的正样本就是item_list去掉第1个和后面2个
            ...
            seq_class_label = self._process_sequence_label_signal(
                seq_label_signal)  # seq_label_signal是每个项目列表的倒数第2个值；转成tensot类型
