...
class Encoder(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, output_all_encoded_layers=True):
        """
        hidden_states:(256, 50, 64),batch_size个用户，每个交互项目embed成64为向量（加上了位置的embedding）
        extend_attention_mask:(256, 1, 50, 50),为attention设置的mask矩阵，每一个用户的mask是50*50。
        因为点乘了下三角，最终有交互的值位于矩阵的右下方，且为0；无交互的为-1000
        """
        all_encoder_layers = []
